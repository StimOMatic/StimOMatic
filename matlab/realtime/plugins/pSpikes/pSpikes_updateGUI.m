...
if ~isempty(transferedGUIData.waveformsToPlot)
    ...
    for jj=1:size(transferedGUIData.waveformsToPlot,1)
        set( handlesPlugin.lineHandles.plotWaveforms(CSCChanNr,  transferedGUIData.handlesToUse(jj) ), ...
            'ydata', transferedGUIData.waveformsToPlot(jj,:)+(CSCChanNr-1)*handlesParent.OSortConstants.plotOffsetWaveforms, 'color', 'r' );
    end
else
    
    %reset them all
    initWaveform=zeros(1,handlesParent.OSortConstants.nrPointsPerWaveform);
    for jj=1:size(handlesPlugin.lineHandles.plotWaveforms,2)
        set( handlesPlugin.lineHandles.plotWaveforms(CSCChanNr,jj), 'ydata', initWaveform+(CSCChanNr-1)*handlesParent.OSortConstants.plotOffsetWaveforms );
    end    
